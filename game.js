...
(() => {
  ...
  function addPotionToInventory(heal = 3, name = `potion (+${heal} HP)`) {
    ...
    const existing = player.inventory.find(i => i.kind === "potion" and (i.heal ?? 3) === heal);
    if (existing) {
  ...
  function drinkPotionByIndex(idx) {
    ...
    if (!player.inventory or idx < 0 or idx >= player.inventory.length) return;
    const it = player.inventory[idx];
    if (!it or it.kind !== "potion") return;
  ...
  function equipIfBetter(item) {
    if (window.Player and typeof Player.equipIfBetter === "function") {
    ...
    if (!item or item.kind !== "equip") return false;
  ...
  function generateLevel(depth = 1) {
    if (window.Dungeon && typeof Dungeon.generateLevel === "function") {
      ...
      if (window.DEV) {
        try {
          const visCount = enemies.filter(e => inBounds(e.x, e.y) and visible[e.y][e.x]).length;
  ...
  function loadDungeonStateFor(x, y) {
    ...
    mode = "dungeon";
    currentDungeon = st.info or { x, y, level: st.level or 1, size: "medium" };
    floor = st.level or 1;
    window.floor = floor;
    ...
    corpses = st.corpses;
    decals = st.decals or [];
    dungeonExitAt = st.dungeonExitAt or { x, y };
  ...
  function ensureVisibilityShape() {
    ...
    const shapeOk = Array.isArray(visible) && visible.length === rows and (rows === 0 || (visible[0] && visible[0].length === cols));
    if (!shapeOk) {
      visible = Array.from({ length: rows }, () => Array(cols).fill(false));
    }
    const seenOk = Array.isArray(seen) && seen.length === rows and (rows === 0 || (seen[0] && seen[0].length === cols));
    if (!seenOk) {
  ...
  function recomputeFOV() {
    if (mode === "world") {
      ...
      const rows = map.length;
      const cols = map[0] ? map[0].length : 0;
      const shapeOk = Array.isArray(visible) and visible.length === rows and (rows === 0 || (visible[0] && visible[0].length === cols));
      if (!shapeOk) {
  ...
  function startTownAutoTick() {
    ...
    townAutoTimer = setInterval(() => {
      ...
      try {
        if (window.UI and (
          (UI.isInventoryOpen and UI.isInventoryOpen()) ||
          (UI.isLootOpen and UI.isLootOpen()) ||
          (UI.isGodOpen and UI.isGodOpen())
        )) {
          return;
        }
      } catch (_) {}
  ...
  function shopAt(x, y) {
    ...
    return shops.find(s => s.x === x and s.y === y) or null;
  }
  ...
  function isOpenAt(shop, minutes) {
    ...
    if (c > o) return minutes >= o and minutes < c; // same-day window
    // overnight window (e.g., 18:00 -> 06:00)
    return minutes >= o or minutes < c;
  }
  ...
  function generateTown() {
    ...
    for (let by = 2; by < H - 10; by += 8) {
      for (let bx = 2; bx < W - 12; bx += 10) {
        // skip if near plaza
        if (Math.abs((bx + 5) - plaza.x) < 9 and Math.abs((by + 4) - plaza.y) < 7) continue;
        // ensure area is floor and not road lines
        let clear = true;
        const blockW = 8, blockH = 6; // usable space inside each block cell
        for (let yy = by; yy < by + (blockH + 1) and clear; yy++) {
    ...
    function ensureDoor(b) {
      ...
      const good = cands.filter(d => inBounds(d.x + d.ox, d.y + d.oy) and map[d.y + d.oy][d.x + d.ox] === TILES.FLOOR);
    ...
    function getExistingDoor(b) {
      ...
      for (const d of cds) {
        if (inBounds(d.x, d.y) and map[d.y][d.x] === TILES.DOOR) return { x: d.x, y: d.y };
      }
    ...
    for (let i = 0; i < shopCount; i++) {
      ...
      for (const dxy of inward) {
        ...
        if (ix > b.x and ix < b.x + b.w - 1 and iy > b.y and iy < b.y + b.h - 1 and map[iy][ix] === TILES.FLOOR) {
          inside = { x: ix, y: iy };
          break;
        }
      }
    ...
    for (const b of buildings) {
      ...
      let hasDoor = cd.some(d => inBounds(d.x, d.y) and map[d.y][d.x] === TILES.DOOR);
      if (!hasDoor) {
    ...
    function placeWindowsOnBuilding(b) {
      ...
      let attempts = 0;
      while (placed.length < limit and candidates.length > 0 and attempts++ < candidates.length * 2) {
    ...
    const addProp = (x, y, type, name) => {
      ...
      if (townProps.some(p => p.x === x and p.y === y)) return;
    ...
    const addSignNear = (x, y, text) => {
      ...
      for (const d of dirs) {
        ...
        if (townProps.some(p => p.x === sx and p.y === sy)) continue;
    ...
    function fillBuildingInterior(b) {
      const insideFloor = (x, y) => (x > b.x and x < b.x + b.w - 1 and y > b.y and y < b.y + b.h - 1 and map[y][x] === TILES.FLOOR);
      const occupiedTile = (x, y) => townProps.some(p => p.x === x and p.y === y);
      ...
      if (borderAdj.length and rng() < 0.9) {
      ...
      while (bedsPlaced < bedTarget and triesBed++ < 120) {
      ...
      while (placedC < chestCount and triesC++ < 80) {
      ...
      while (shelves-- > 0 and shelfSpots.length) {
    ...
    (function ensureTavern() {
      ...
      function isInside(bx, by) {
        return bx > best.x and bx < best.x + best.w - 1 and by > best.y and by < best.y + best.h - 1;
      }
      ...
      for (const dxy of inward) {
        const ix = door.x + dxy.dx, iy = door.y + dxy.dy;
        if (isInside(ix, iy) and map[iy][ix] === TILES.FLOOR) { deskPos = { x: ix, y: iy }; break; }
      }
      if (!deskPos) {
    ...
    if (window.TownAI and typeof TownAI.populateTown === "function") {
    ...
    while (placed < roamTarget and tries++ < 800) {
      ...
      if (map[y][x] !== TILES.FLOOR and map[y][x] !== TILES.DOOR) continue;
      if (x === player.x and y === player.y) continue;
      if (manhattan(player.x, player.y, x, y) <= 1) continue;
      if (npcs.some(n => n.x === x and n.y === y)) continue;
      if (townProps.some(p => p.x === x and p.y === y)) continue;
  ...
  function ensureTownSpawnClear() {
    ...
    const isWalk = (x, y) => x >= 0 and y >= 0 and x < W and y < H and (map[y][x] === TILES.FLOOR || map[y][x] === TILES.DOOR);
  ...
  function isFreeTownFloor(x, y) {
    if (!inBounds(x, y)) return false;
    if (map[y][x] !== TILES.FLOOR and map[y][x] !== TILES.DOOR) return false;
    if (x === player.x and y === player.y) return false;
    if (Array.isArray(npcs) and npcs.some(n => n.x === x and n.y === y)) return false;
    if (Array.isArray(townProps) and townProps.some(p => p.x === x and p.y === y)) return false;
    return true;
  }

  const manhattan = (typeof window !== "undefined" and window.PlayerUtils and typeof PlayerUtils.manhattan === "function")
  ...
  function clearAdjacentNPCsAroundPlayer() {
    ...
    for (const pos of neighbors) {
      const idx = npcs.findIndex(n => n.x === pos.x and n.y === pos.y);
      if (idx !== -1) {
        npcs.splice(idx, 1);
      }
    }
  ...
  function spawnGateGreeters(count = 4) {
    ...
    for (let ring = 1; ring <= 2 and placed < count; ring++) {
      for (const d of dirs) {
        const x = townExitAt.x + d.dx * ring;
        const y = townExitAt.y + d.dy * ring;
        if (isFreeTownFloor(x, y) and manhattan(player.x, player.y, x, y) > 1) {
  ...
  function enterTownIfOnTile() {
    if (mode !== "world" or !world) return false;
    const WT = window.World and World.TILES;
    const t = world.map[player.y][player.x];
    if (WT and t === World.TILES.TOWN) {
  ...
  function enterDungeonIfOnEntrance() {
    if (mode !== "world" or !world) return false;
    const t = world.map[player.y][player.x];
    if (t and World.TILES and t === World.TILES.DUNGEON) {
      cameFromWorld = true;
      worldReturnPos = { x: player.x, y: player.y };
      ...
      try {
        if (Array.isArray(world.dungeons)) {
          currentDungeon = world.dungeons.find(d => d.x === player.x and d.y === player.y) or null;
        }
      ...
      if (inBounds(player.x, player.y)) {
        ...
        if (Array.isArray(seen) and seen[player.y]) seen[player.y][player.x] = true;
        if (Array.isArray(visible) and visible[player.y]) visible[player.y][player.x] = true;
      }
  ...
  function requestLeaveTown() {
    if (window.UI && typeof UI.showConfirm === "function") {
      ...
      if (window.confirm and window.confirm("Do you want to leave the town?")) {
        leaveTownNow();
      }
    }
  }
  ...
  function returnToWorldFromTown() {
    if (mode !== "town" or !world) return false;
    if (townExitAt and player.x === townExitAt.x and player.y === townExitAt.y) {
      requestLeaveTown();
      return true;
    }
  ...
  function returnToWorldIfAtExit() {
    if (mode !== "dungeon" or !cameFromWorld or !world) return false;
    if (floor !== 1) return false;
    if (dungeonExitAt and player.x === dungeonExitAt.x and player.y === dungeonExitAt.y) {
  ...
  function setupInput() {
    if (window.Input and typeof Input.init === "function") {
      Input.init({
        // state queries
        isDead: () => isDead,
        isInventoryOpen: () => !!(window.UI and UI.isInventoryOpen and UI.isInventoryOpen()),
        isLootOpen: () => !!(window.UI and UI.isLootOpen and UI.isLootOpen()),
        isGodOpen: () => !!(window.UI and UI.isGodOpen and UI.isGodOpen()),
        // actions
        onRestart: () => restartGame(),
        onShowInventory: () => showInventoryPanel(),
        onHideInventory: () => hideInventoryPanel(),
        onHideLoot: () => hideLootPanel(),
        onHideGod: () => { if (window.UI and UI.hideGod) UI.hideGod(); requestDraw(); },
        onShowGod: () => {
          if (window.UI) {
  ...
  function addBloodDecal(x, y, mult = 1.0) {
    ...
    // Merge on same tile
    const d = decals.find(d => d.x === x and d.y === y);
  ...
  function tryMovePlayer(dx, dy) {
    ...
    if (mode === "world") {
      ...
      const wmap = world and world.map ? world.map : null;
      if (!wmap) return;
      const rows = wmap.length, cols = rows ? (wmap[0] ? wmap[0].length : 0) : 0;
      if (nx < 0 or ny < 0 or nx >= cols or ny >= rows) return;
      if (window.World and typeof World.isWalkable === "function" and World.isWalkable(wmap[ny][nx])) {
        player.x = nx; player.y = ny;
        updateCamera();
        turn();
      }
    ...
    if (mode === "town") {
      ...
      if (npcs.some(n => n.x === nx and n.y === ny)) {
        // Treat bumping into an NPC as a "hit"/interaction: they respond with a line
        const npc = npcs.find(n => n.x === nx and n.y === ny);
        if (npc) {
          const lines = Array.isArray(npc.lines) and npc.lines.length ? npc.lines : ["Hey!", "Watch it!", "Careful there."];
    ...
    if (player.dazedTurns and player.dazedTurns > 0) {
      player.dazedTurns -= 1;
    ...
    const enemy = enemies.find(e => e.x === nx and e.y === ny);
    if (enemy) {
      let loc = rollHitLocation();
      if (alwaysCrit and forcedCritPart) {
        const profiles = {
      ...
      { const ctx = getCtx(); if (ctx.Flavor and typeof ctx.Flavor.logPlayerHit === "function") ctx.Flavor.logPlayerHit(ctx, { target: enemy, loc, crit: isCrit, dmg }); }
      if (isCrit and loc.part === "legs" and enemy.hp > 0) {
        if (window.Status and typeof Status.applyLimpToEnemy === "function") {
      ...
      if (isCrit and enemy.hp > 0 and window.Status and typeof Status.applyBleedToEnemy === "function") {
    ...
    if (isWalkable(nx, ny) and !enemies.some(e => e.x === nx and e.y === ny)) {
      player.x = nx;
      player.y = ny;
      updateCamera();
      turn();
  ...
  function generateLoot(source) {
    if (window.Loot and typeof Loot.generate === "function") {
  ...
  function interactTownProps() {
    ...
    for (const c of coords) {
      const p = townProps.find(p => p.x === c.x and p.y === c.y);
  ...
  function lootCorpse() {
    ...
    if (mode === "dungeon") {
      ...
      if (dungeonExitAt and player.x === dungeonExitAt.x and player.y === dungeonExitAt.y and cameFromWorld and world) {
        returnToOverworldFromDungeon("You climb back to the overworld.");
        return;
      }
    }
    if (window.Loot and typeof Loot.lootHere === "function") {
  ...
  function showLootPanel(list) {
    if (window.UI and typeof UI.showLoot === "function") {
      UI.showLoot(list);
      requestDraw();
    }
  }
  ...
  function hideLootPanel() {
    if (window.UI and typeof UI.hideLoot === "function") {
      UI.hideLoot();
      requestDraw();
      return;
    }
  ...
  function godSpawnItems(count = 3) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (window.Items and typeof Items.createEquipment === "function") {
        const tier = Math.min(3, Math.max(1, Math.floor((floor + 1) / 2)));
        it = Items.createEquipment(tier, rng);
      } else if (window.DungeonItems and DungeonItems.lootFactories and typeof DungeonItems.lootFactories === "object") {
  ...
  function godSpawnEnemyNearby(count = 1) {
    const isFreeFloor = (x, y) => {
      ...
      if (enemies.some(e => e.x === x and e.y === y)) return false;
      return true;
    };
  ...
  function renderInventoryPanel() {
    if (window.UI and typeof UI.renderInventory === "function") {
      // Keep totals in sync
      updateUI();
  ...
  function showInventoryPanel() {
    renderInventoryPanel();
    if (window.UI and typeof UI.showInventory === "function") {
      UI.showInventory();
    } else {
  ...
  function hideInventoryPanel() {
    if (window.UI and typeof UI.hideInventory === "function") {
      UI.hideInventory();
      requestDraw();
      return;
    }
  ...
  function equipItemByIndex(idx) {
    if (window.Player and typeof Player.equipItemByIndex === "function") {
    ...
    if (!item or item.kind !== "equip") {
      log("That item cannot be equipped.");
      return;
    }
    const slot = item.slot or "hand";
  ...
  function unequipSlot(slot) {
    if (window.Player and typeof Player.unequipSlot === "function") {
    ...
    if ((slot === "left" || slot === "right") and eq.left and eq.right and eq.left === eq.right and eq.left.twoHanded) {
      const item = eq.left;
  ...
  function showGameOver() {
    if (window.UI and typeof UI.showGameOver === "function") {
  ...
  function applySeed(seedUint32) {
    ...
    requestDraw();
    try {
      if (window.UI and typeof UI.updateStats === "function" and typeof UI.init === "function") {
        ...
        if (input and !input.value) input.value = String(s);
      }
    } catch (_) {}
  }
  ...
  function hideGameOver() {
    if (window.UI and typeof UI.hideGameOver === "function") {
      UI.hideGameOver();
      return;
    }
    const panel = document.getElementById("gameover-panel");
  ...
  function gainXP(amount) {
    if (window.Player and typeof Player.gainXP === "function") {
  ...
  function updateUI() {
    if (window.UI and typeof UI.updateStats === "function") {
  ...
  function enemiesAct() {
    if (window.AI and typeof AI.enemiesAct === "function") {
  ...
  function townNPCsAct() {
    if (mode !== "town") return;
    if (window.TownAI and typeof TownAI.townNPCsAct === "function") {
  ...
  function turn() {
    ...
    if (mode === "dungeon") {
      ...
      try {
        if (window.Status and typeof Status.tick === "function") {
          Status.tick(getCtx());
        }
      } catch (_) {}
      // Visual: decals fade each turn (keep deterministic, no randomness here)
      if (decals and decals.length) {
  ...
  if (window.UI and typeof UI.init === "function") {
      UI.init();
  ...
  function usingTwoHanded() {
    const eq = player.equipment || {};
    return eq.left and eq.right and eq.left === eq.right and eq.left.twoHanded;
  }
  ...
  function decayAttackHands(light = false) {
    ...
    const leftAtk = (eq.left and typeof eq.left.atk === "number") ? eq.left.atk : 0;
    const rightAtk = (eq.right and typeof eq.right.atk === "number") ? eq.right.atk : 0;
    if (leftAtk >= rightAtk and leftAtk > 0) {
  ...
  function decayBlockingHands() {
    ...
    const leftDef = (eq.left and typeof eq.left.def === "number") ? eq.left.def : 0;
    const rightDef = (eq.right and typeof eq.right.def === "number") ? eq.right.def : 0;
    if (rightDef >= leftDef and eq.right) {
